...
action_5 (14) = happyShift action_16
...
action_14 (11) = happyShift action_33
action_14 _ = happyFail

action_15 (12) = happyShift action_32
...
action_16 (8) = happyGoto action_31
...
action_17 (8) = happyGoto action_30
...
happyReduction_3 (HappyAbsSyn5  happy_var_3)
	...
	(HappyAbsSyn7  happy_var_1)
	 =  HappyAbsSyn5
		 (Listspec happy_var_1 happy_var_3
	)
...
happyReduction_6 (_ `HappyStk`
	...
	happyRest)
	 = HappyAbsSyn6
		 (AppE (VarE (mkName "\\interval")) (VarE happy_var_4)
	) `HappyStk` happyRest
...
happyReduction_7 (_ `HappyStk`
	...
	happyRest)
	 = HappyAbsSyn6
		 (TupE [LitE (IntegerL happy_var_2), LitE (IntegerL happy_var_4)]
	) `HappyStk` happyRest
...
happyReduction_10 (HappyTerminal (TokenBool happy_var_1))
	 =  HappyAbsSyn8
		 ((ConE happy_var_1)
	)
...
happyReduction_11 (HappyAbsSyn8  happy_var_3)
	...
	(HappyAbsSyn8  happy_var_1)
	 =  HappyAbsSyn8
		 ((InfixE (listToMaybe [happy_var_1]) (VarE (mkName "||"))  (listToMaybe [happy_var_3]))
	)
...
happyNewToken action sts stk (tk:tks) =
	...
	TokenAND -> cont 15;
	TokenEquals -> cont 16;
	TokenGreater -> cont 17;
	TokenLesser -> cont 18;
	TokenGreaterEq -> cont 19;
	TokenLesserEq -> cont 20;
	TokenWCET -> cont 21;
	TokenBackS -> cont 22;
	TokenInterval -> cont 23;
	TokenLeftSB -> cont 24;
	TokenRightSB -> cont 25;
	TokenComma -> cont 26;
...
whileParser tks = happySomeParser where
...
data Program  = Seqspec Listspec deriving Show
data Listspec = Spec Boolean | Listspec Boolean Listspec deriving Show
data Boolean  =  Boolean Exp deriving Show
...
lexer ('\\':cs) = TokenBackS : lexer cs
lexer ('[':cs) = TokenLeftSB : lexer cs
lexer (']':cs) = TokenRightSB : lexer cs
...
lexVar cs =
   case span isAlpha cs of
	("True",rest) -> TokenBool (mkName "True") : lexer rest
	("False",rest) -> TokenBool (mkName "False") : lexer rest
	("wcet",rest) -> TokenWCET: lexer rest
	("interval",rest) -> TokenInterval: lexer rest
	(var,rest)   -> TokenVar (mkName var) : lexer rest


runWhile :: String -> Program
runWhile s
 = case whileParser (lexer s) 1 of
	    ParseOk e -> e
	    ParseFail s -> error s


happyError = \tks i -> error (
	"Parse error in lines " ++  show (i::Int) ++ "\n")
