...
module Data.LinearProgram.GLPK.IO.Internal (readGLPLP, writeGLPLP, getNumRows, getNumCols, getRows, rowBounds, colBounds, getObjCoef) where
...
foreign import ccall unsafe "c_glp_get_num_nz" glpGetNumNz :: Ptr GlpProb -> IO CInt
...
getNumNz :: GLPK Int
getNumNz = liftM (fromIntegral) $ GLP glpGetNumNz
...
getRows = do	n <- getNumRows
		...
		sequence [do
			...
			coefsL <- liftIO $ mapM (peekElemOff coefs) [1..k]
