...
module Data.LinearProgram.GLPK.Internal (writeProblem, solveSimplex, mipSolve,
	getObjVal,  getRowPrim, getColPrim, getColDual, solveInternal, getStatus, getPrimStatus, getDualStatus,
	iptObjVal,  iptColPrim, iptColDual, iptStatus, getRowDual,
    mipObjVal, mipRowVal, mipColVal, getBadRay) where
...
foreign import ccall unsafe "c_glp_solve_internal" glpSolveInternal :: Ptr GlpProb -> IO CInt
foreign import ccall unsafe "c_glp_get_status" glpGetStatus :: Ptr GlpProb -> IO CInt
foreign import ccall unsafe "c_glp_get_prim_stat" glpGetPrimStatus :: Ptr GlpProb -> IO CInt
foreign import ccall unsafe "c_glp_get_dual_stat" glpGetDualStatus :: Ptr GlpProb -> IO CInt
foreign import ccall unsafe "c_glp_ipt_status" iptGetStatus :: Ptr GlpProb -> IO CInt

foreign import ccall unsafe "c_glp_get_obj_val" glpGetObjVal :: Ptr GlpProb -> IO CDouble
foreign import ccall unsafe "c_glp_ipt_obj_val" glpIptObjVal :: Ptr GlpProb -> IO CDouble
...
foreign import ccall unsafe "c_glp_get_col_dual" glpGetColDual :: Ptr GlpProb -> CInt -> IO CDouble
foreign import ccall unsafe "c_glp_get_row_dual" glpGetRowDual :: Ptr GlpProb -> CInt -> IO CDouble

foreign import ccall unsafe "c_glp_ipt_col_prim" glpIptColPrim :: Ptr GlpProb -> CInt -> IO CDouble
foreign import ccall unsafe "c_glp_ipt_col_dual" glpIptColDual :: Ptr GlpProb -> CInt -> IO CDouble
...
solveInternal :: GLPK ReturnCode
solveInternal = GLP $ \ lp -> liftM (toEnum . fromIntegral) $ glpSolveInternal lp

getStatus :: GLPK ReturnCode
getStatus = GLP $ \ lp -> liftM (toEnum . fromIntegral) $ glpGetStatus lp

getPrimStatus :: GLPK ReturnCode
getPrimStatus = GLP $ \ lp -> liftM (toEnum . fromIntegral) $ glpGetPrimStatus lp

getDualStatus :: GLPK ReturnCode
getDualStatus = GLP $ \ lp -> liftM (toEnum . fromIntegral) $ glpGetDualStatus lp

iptStatus :: GLPK ReturnCode
iptStatus = GLP $ \ lp -> liftM (toEnum . fromIntegral) $ iptGetStatus lp
...
iptObjVal :: GLPK Double
iptObjVal = liftM realToFrac $ GLP glpIptObjVal
...
getColDual :: Int -> GLPK Double
getColDual i = liftM realToFrac $ GLP (`glpGetColDual` fromIntegral i)

getRowDual :: Int -> GLPK Double
getRowDual i = liftM realToFrac $ GLP (`glpGetRowDual` fromIntegral i)

iptColPrim :: Int -> GLPK Double
iptColPrim i = liftM realToFrac $ GLP (`glpIptColPrim` fromIntegral i)

iptColDual :: Int -> GLPK Double
iptColDual i = liftM realToFrac $ GLP (`glpIptColDual` fromIntegral i)
