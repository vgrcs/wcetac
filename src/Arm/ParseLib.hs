...
module Arm.ParseLib
   (Parser, item, papply, (+++), sat, many, many1, sepby, sepby1, chainl,
    chainl1, chainr, chainr1, ops, bracket, char, digit, lower, upper, label,
    letter, alphanum, prop, sentence, string, ident, nat, int, spaces, comment, junk,
...
prop              :: Parser Char
prop              =  sat (\c -> c == '_' || c == '<' || c == '>' ||  isAlphaNum c)

label             :: Parser Char
label             = sat (\c -> c == '_' || c == '-' || c == '(' || c == ')' || isAlphaNum c )

sentence          :: Parser Char
sentence          =  sat (\c -> isPunctuation c || isSeparator c || isAlphaNum c || isSymbol c)
...
int               :: Parser Integer
int                = (do {char '-'; n <- nat; return (-(toInteger n))})
                     +++
                     (do {n <- nat; return (toInteger n)})
