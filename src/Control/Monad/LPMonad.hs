...
{-# SPECIALIZE quickSolveLP :: (Ord v, Show v, Real c) => String ->
	LPT v c IO (ReturnCode, Maybe (Double, Map v Double, Map v Double, Map Int Double, Map Int Double)) #-}
{-# SPECIALIZE quickSolveMIP :: (Ord v, Show v, Real c) => String ->
	LPT v c IO (ReturnCode, Maybe (Double,Map v Double, Map v Double, Map Int Double, Map Int Double)) #-}
...
quickSolveLP, quickSolveMIP :: (Ord v, Show v, Real c, MonadState (LP v c) m, MonadIO m) => String ->
	m (ReturnCode, Maybe (Double, Map v Double, Map v Double,Map Int Double, Map Int Double))
quickSolveLP file = glpSolve simplexDefaults file
quickSolveMIP file = glpSolve mipDefaults file

{-# SPECIALIZE glpSolve :: (Ord v, Real c, Show v) => GLPOpts -> String -> LPT v c IO (ReturnCode, Maybe (Double, Map v Double, Map v Double,Map Int Double, Map Int Double)) #-}
-- | Solves the linear program with the specified options in GLPK.  Returns the return code,
-- and if the solver was successful, the objective function value and the settings of each variable.
glpSolve :: (Ord v, Show v, Real c, MonadState (LP v c) m, MonadIO m) => GLPOpts -> String -> m (ReturnCode, Maybe (Double, Map v Double, Map v Double, Map Int Double, Map Int Double))
glpSolve opts file = get >>= liftIO . glpSolveVars opts file


{-# SPECIALIZE glpMatrix :: (Ord v, Real c, Show v) => GLPOpts -> LPT v c IO (ReturnCode, Maybe (Int, Int, [(Int, [(Int, Double)])], [(Int, Bounds Double)], [(Int, Double)], Map v Int)) #-}

glpMatrix :: (Ord v, Show v, Real c, MonadState (LP v c) m, MonadIO m) => GLPOpts -> m (ReturnCode, Maybe (Int, Int, [(Int, [(Int, Double)])], [(Int, Bounds Double)] , [(Int, Double)], Map v Int))
glpMatrix opts = get >>= liftIO . glpSolveMatrix opts
